...
const HF_MODELS = [
  'llava-hf/llava-1.5-7b-hf',  // Best for charts
  'llava-hf/llava-13b-hf',     // More powerful fallback
];

const HF_TIMEOUT_MS = Number(process.env.HF_TIMEOUT_MS || 40_000);
...
function classifyProviderError(message, statusCode = 502) {
  ...
  if (lower.includes('rate') || lower.includes('quota')) {
    return Object.assign(new Error(text), {
      statusCode: 429,
      publicMessage: 'Hugging Face is rate limited. Please retry in a moment.',
      retryable: true,
    });
  }
  ...
  if (lower.includes('unavailable') || lower.includes('overload') || statusCode === 503 || statusCode === 429) {
    return Object.assign(new Error(text), {
      statusCode: 503,
      publicMessage: 'Hugging Face is temporarily busy. Your image uploaded correctly; retry in a moment.',
      retryable: true,
    });
  }

  return Object.assign(new Error(text || 'Hugging Face request failed.'), {
...
async function requestHF({ model, base64Image, extraData, prompt, signal }) {
  ...
  if (!response.ok) {
    const message = data?.error || data?.message || `Hugging Face request failed with status ${response.status}`;
    throw classifyProviderError(message, response.status);
  }

  // HF returns array, extract text
  let text = '';
  if (Array.isArray(data)) {
    text = data.map((item) => item?.generated_text || '').join('');
  } else {
    text = data?.generated_text || '';
  }

  if (!text || text.length < 50) {
    throw Object.assign(new Error('Hugging Face returned insufficient analysis.'), {
      statusCode: 502,
      publicMessage: 'The AI returned insufficient analysis. Please retry with a full chart screenshot.',
      retryable: true,
    });
  }
...
export async function analyzeChart(imagePath, extraData = {}) {
  if (!HF_API_KEY) {
    throw Object.assign(new Error('Hugging Face API key is not configured.'), { statusCode: 500 });
  }
...
Analyze only what is visible in the chart screenshot. Do not invent indicators, prices, timeframes, or patterns that cannot be inferred from the image.
...
OCR context: ${extraData.ocrText || 'No OCR text extracted.'}
BTC Context: ${JSON.stringify(extraData.marketContext || {})}`;

  const request = withTimeout(async (signal) => {
    let lastError;
    for (const model of HF_MODELS) {
      try {
        return await requestHF({ model, base64Image, extraData, prompt, signal });
      } catch (error) {
        lastError = error;
        console.error(`[huggingface:${model}]`, error.publicMessage || error.message);
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw Object.assign(new Error('Hugging Face analysis timed out. Try a clearer or smaller chart image.'), { statusCode: 504 });
    }
    throw error;
  }
}
