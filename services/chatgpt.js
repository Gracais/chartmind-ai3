...
const OPENAI_TIMEOUT_MS = Number(process.env.OPENAI_TIMEOUT_MS || 40_000);
...
async function requestOpenAI({ base64Image, extraData, prompt, signal }) {
...
BTC Context: ${JSON.stringify(extraData.marketContext || {})}`;
  ...
  const request = withTimeout(async (signal) => {
    try {
      return await requestOpenAI({ base64Image, extraData, prompt, signal });
    } catch (error) {
